...
export function ScheduleGameCard({
  ...
  return (
    <div className={`mb-3 px-4 py-4 rounded-[28px] bg-white flex items-center gap-4 ${past ? 'opacity-75' : ''}`}>
          ...
          <div className="max-w-2xl mx-auto">
            {upcoming.map((game) => (
              <div key={game.id} className="mx-4">
                <ScheduleGameCard game={game} past={false} editHref={editFor?.(game, false) ?? null} />
              </div>
            ))}
          </div>
        </section>
      ...
      {past.length > 0 && (
        <section>
          ...
          <div className="max-w-2xl mx-auto">
            {past.map((game) => (
              <div key={game.id} className="mx-4">
                <ScheduleGameCard game={game} past editHref={editFor?.(game, true) ?? null} />
              </div>
            ))}
          </div>
        </section>
