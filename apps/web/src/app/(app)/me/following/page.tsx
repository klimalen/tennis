...
import { FollowersList, type FollowerProfile } from '../followers/FollowersList'
...
async function FollowingContent() {
  ...
  const following = (rows ?? []).map((r) => r.profiles as unknown as FollowerProfile).filter(Boolean)
    ...
    <div className="min-h-screen pb-20 md:pb-0">
      <div className="sticky top-0 bg-brand-bg/95 backdrop-blur-sm z-10 px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <span className="font-display text-3xl tracking-wide">FOLLOWING</span>
          <span className="text-[rgba(26,26,26,0.35)] text-sm font-numbers">{following.length}</span>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        {following.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
            ...
            <p className="text-[10px] tracking-[0.2em] uppercase text-[rgba(26,26,26,0.4)] mb-1">Not following anyone yet</p>
            <p className="text-xs font-fraunces italic text-[#85648F]">People you follow appear here</p>
          </div>
        ) : (
